...
def compute_candidate(smile, smiles_selected, smiles_train, smiles_pool, mhfp_df, pool_df):
    ...
    mean_desirability_score = pool_df[pool_df['SMILES'].isin(temp_selected)]['geometric_predicted_score'].mean()
    combined_score = coverage_score * mean_desirability_score
    return (smile, combined_score)

def desired_coverage(self, batch_size):
